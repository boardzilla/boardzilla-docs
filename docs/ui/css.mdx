...
The Board is rendered here, as opposed to any player controls. Use this in case
of CSS selectors that must be scoped to the board only.
...
If the element is currently zoomed, this class will also have a `data-zoomed`
attribute set. It is sometimes necessary to target this to apply styles only to
the non-zoomed or zoomed versions of a Piece, e.g.:

```css
.transform-wrapper[data-zoomed] [
```
### .Space or .Piece
